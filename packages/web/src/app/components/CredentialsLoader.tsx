...
export default function CredentialsLoader({ reason, onReady, onStep, compact = false }: { reason?: string; onReady?: () => void; onStep?: (step: SignInStep) => void; compact?: boolean }) {
  ...
  return (
    <div className={compact ? '' : 'grid gap-3'} aria-busy={busy}>
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        <button type="button" aria-expanded={showPaste} aria-controls={`${id}-paste`} onClick={() => setShowPaste((v) => !v)} className="text-[14px] text-muted transition-colors hover:text-text">
          {showPaste ? 'Hide paste box' : 'Paste JSON instead'}
        </button>
      </div>
      {showPaste ? (
        <div id={`${id}-paste`} className="grid gap-2">
          <label htmlFor={`${id}-json`} className="text-[14px] text-text">Credentials JSON</label>
          <textarea
            id={`${id}-json`}
      ...
      {error ? <p id={`${id}-error`} role="alert" className="text-[14px] text-bad">{error}</p> : null}
