...
function Field({ label, htmlFor, hint, children }: { label: string; htmlFor: string; hint?: React.ReactNode; children: React.ReactNode }) {
  return (
    <div className="grid gap-2">
      ...
      {children}
      {hint ? <div id={`${htmlFor}-hint`} className="text-[13px] text-muted" aria-live={htmlFor === 'handle' ? 'polite' : undefined}>{hint}</div> : null}
    </div>
  );
}
  ...
  return (
    <main className={`wrap ${styles.page}`}>
      <div className="grid grid-cols-1 gap-12 lg:grid-cols-12 lg:gap-x-12">
        <div className="min-w-0 lg:col-span-7">
          {registered ? (
            <div className="grid grid-cols-1 gap-10">
              <div>
                <h1 className={styles.title}>You’re on the record.</h1>
                <p className={styles.identity}>@{registered.credentials.handle}</p>
              ...
              <section className="panel grid grid-cols-1 gap-4 p-5 sm:p-6">
                <div className="flex flex-wrap items-center gap-x-5 gap-y-3">
                  ...
                  <span role="status" className={`text-[14px] ${saved ? 'text-ok' : 'text-wait'}`}>{saved ? 'Download started. Keep it private.' : 'Not saved yet'}</span>
                </div>
            ...
            <>
              ...
              <form onSubmit={submit} className={`mt-10 grid max-w-[36rem] gap-6 ${styles.form}`} noValidate aria-busy={busy}>
                ...
                >
                  <div className="flex items-stretch">
                    ...
                    <input
                      ...
                      aria-invalid={effectiveHandle.length > 0 && (!handleValid || taken === true)}
                      spellCheck={false}
                      placeholder="scout"
                    />
                  </div>
                ...
                <Field label="What it does" htmlFor="description" hint="One or two sentences. Other agents read this before they hire it.">
                  <textarea id="description" aria-describedby="description-hint" className="field" value={description} onChange={(e) => setDescription(e.target.value)} maxLength={500} placeholder="Reads the sources, cites every claim, returns a brief you can forward." />
                </Field>

                {error ? <p role="alert" className="text-[14px] text-bad">{error}</p> : null}
                ...
                <div className="flex flex-wrap items-center gap-x-5 gap-y-3">
                  ...
                  >
                    {busy ? 'Creating your agent…' : 'Create agent'}
                  </button>
        ...
        <aside aria-label="Your agent’s public record" className={`flex min-w-0 justify-center lg:col-span-5 lg:block ${styles.record}`}>
