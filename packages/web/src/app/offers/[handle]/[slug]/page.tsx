      ...
      {props.map(([key, p]) => (
        <li key={key} className="grid grid-cols-1 gap-x-6 gap-y-0.5 px-4 py-3 sm:grid-cols-[minmax(0,11rem)_minmax(0,9rem)_minmax(0,1fr)]">
          <span className="figure truncate text-[14px] text-text" title={key}>
            {key}
            {required.has(key) ? '' : <span className="text-dim">?</span>}
          </span>
          <span className="figure truncate text-[13px] text-muted" title={typeOf(p)}>
            {typeOf(p)}
          </span>
          <span className="text-[14px] text-muted">{p.description ?? (required.has(key) ? 'required' : 'optional')}</span>
        </li>
      ))}
    </ul>
  );
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const description = `${priceLabel(offer.priceMicros)}. Takes ${offer.inputFields.join(', ') || 'nothing'}, returns ${offer.outputFields.join(', ') || 'nothing'}. ${offer.description}`.slice(0, 200);
    ...
    <main className="wrap pb-24 pt-10 sm:pt-14">
      <div className="grid grid-cols-1 gap-12 lg:grid-cols-12 lg:gap-x-12">
        <div className="grid min-w-0 grid-cols-1 content-start gap-12 lg:col-span-8">
          <header>
            ...
            <p className="mt-3 flex flex-wrap gap-x-3 gap-y-1 text-[15px] text-muted">
              ...
              <span>
                ...
                {owner.isHouse ? ', run by the registry' : ''}
              </span>
          ...
          <SchemaBlock title="Takes" schema={offer.inputSchema} url={offer.urls.inputSchema} />
          <SchemaBlock title="Returns" schema={offer.outputSchema} url={offer.urls.outputSchema} />

          {example ? (
            <section className="grid gap-3">
              <h2 className="text-[15px] font-medium text-text">Example</h2>
              <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
                <div className="flex min-w-0 flex-col">
                  <p className="mb-1.5 text-[12px] text-dim">input</p>
                  <pre className="figure max-h-[340px] flex-1 overflow-auto whitespace-pre rounded-sm border border-ink-3 bg-floor p-3 text-[12px] leading-[1.55] text-text">{JSON.stringify(example.input, null, 2)}</pre>
                </div>
                <div className="flex min-w-0 flex-col">
                  <p className="mb-1.5 text-[12px] text-dim">output</p>
                  <pre className="figure max-h-[340px] flex-1 overflow-auto whitespace-pre rounded-sm border border-ink-3 bg-floor p-3 text-[12px] leading-[1.55] text-text">{JSON.stringify(example.output, null, 2)}</pre>
                </div>
              </div>
            </section>
          ) : null}
          ...
          <section className="grid grid-cols-1 gap-3">
            <h2 className="text-[15px] font-medium text-text">Call it</h2>
            <p className="max-w-[40rem] text-[14px] text-muted">From any MCP client, as one tool. Or over HTTP with an API key. Either way the registry validates the input, holds the price, checks the output against the schema and seals the receipt.</p>
            ...
            <CopyLine label="skill" value={offer.urls.skill} />
          </section>

          <section className="grid gap-3">
            <h2 className="text-[15px] font-medium text-text">Try it here</h2>
            <TryOffer name={label} example={example?.input ?? {}} priceMicros={offer.priceMicros} paused={paused} />
          </section>

          {feeds.length > 0 || fedBy.length > 0 ? (
            <section className="grid gap-3">
              <h2 className="text-[15px] font-medium text-text">Composes with</h2>
              <p className="max-w-[40rem] text-[14px] text-muted">Offers whose schemas line up with this one, declared by the owner or matched on field types.</p>
              <ul className="grid gap-2 text-[14px]">
                {feeds.map((o) => (
                  <li key={`f-${o.id}`} className="flex flex-wrap gap-x-3">
                    <span className="text-muted">output feeds</span>
                ...
                {fedBy.map((o) => (
                  <li key={`b-${o.id}`} className="flex flex-wrap gap-x-3">
                    <span className="text-muted">input from</span>
                    <Link className="link figure" href={offerPath(o.name)}>
                      {offerLabel(o.name)}
                    </Link>
                    <span className="text-dim">{o.title}</span>
        ...
        <aside className="grid min-w-0 content-start justify-items-center gap-6 lg:col-span-4 lg:justify-items-stretch">
          <div className="paper-shadow w-full max-w-[360px] lg:sticky lg:top-24 lg:ml-auto">
            <div className="paper torn-b px-6 pb-10 pt-5">
              <div className="flex items-center justify-between gap-4">
                <span className="receipt-head">ANS offer</span>
                <Mark size={18} className={offer.probeOk ? 'text-paper-ink' : 'text-paper-muted'} title={offer.probeOk ? 'probe passed' : 'not probed'} />
              </div>
              ...
              {offer.priceMicros !== '0' ? (
                <div className="paper-row">
                  <span>fee</span>
                  <span>{bpsPercent(FEE_BPS)}, from the price</span>
                </div>
              ) : null}
              <div className="paper-row">
                <span>sandbox credit</span>
                <span>{offer.acceptsSandbox ? 'accepted' : 'cash only'}</span>
              </div>
              <div className="paper-row">
                <span>timeout</span>
              ...
              <div className="paper-row">
                <span>succeeded</span>
                <span>{okRate === null ? 'no calls yet' : `${okRate}%`}</span>
              </div>
              <div className="paper-row">
                <span>bad output</span>
                <span className={offer.stats.outputInvalid > 0 ? '!text-paper-bad' : ''}>{offer.stats.outputInvalid}</span>
              </div>
              <div className="paper-row">
                <span>latency p50 · p95</span>
                <span>{offer.stats.p50Ms === null ? 'n/a' : `${offer.stats.p50Ms} · ${offer.stats.p95Ms ?? '-'} ms`}</span>
              </div>
              <div className="paper-row">
                <span>last call</span>
              ...
              <div className="paper-row">
                ...
                <span>{owner.isHouse ? 'house, unranked' : `${owner.trust.score} · ${confidenceLabel(owner.trust.confidence)}`}</span>
              </div>
              <div className="paper-row">
                <span>probe</span>
                <span>{offer.probedAt ? `${offer.probeOk ? 'passed' : 'failed'} ${isoDate(offer.probedAt)}` : 'not probed'}</span>
              </div>
              <div className="paper-row">
          ...
          <p className="w-full max-w-[360px] text-[13px] text-dim lg:ml-auto">
            {plural(offer.stats.calls, 'call')} so far. Every one is a receipt on {owner.handle ? `@${owner.handle}` : 'the owner'}’s record.
          </p>
        </aside>
      </div>
    </main>
