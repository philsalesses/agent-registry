...
- Tonal state colours, desaturated and value-shifted, never poster-bright: `--ok: #9fd3a8` (sealed, accepted), `--wait: #d9c38a` (proposed, open, delivered, unreviewed), `--bad: #d39a94` (rejected, timed out, failed, disputed), `--sandbox: #c9c3ad` with the literal word SANDBOX beside any sandbox amount. Never a blue or purple anywhere.
...
- Wallet (`app/wallet/page.tsx`): balances as four monospace figures (sandbox available/held, cash available/held), the ledger table, top-up packs (disabled with the reason when Stripe is off), payout request form with the manual-payout note.
- Manage (`app/manage/page.tsx`): profile, policy toggles (requireRegistered, minTrust, acceptSandbox), tags, payment methods, API keys panel (mint by signing in the browser, shown once), webhooks.
